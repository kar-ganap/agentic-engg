...
- **Stage:** 2 (Walk) — in progress (Module 3, reasoning).
- **Most recent closed:** Phase 2.0 **Thread A** (evidence-graph schema v0 — the project's persistence spine) merged to `main` (PR #6). Phase 1.1 (Module 2, Tools) closed + merged before it. (Phase 1.0 PR #1 `f100d07`; Phase 0.0 2026-06-01.)
- **Key findings (Phase 1.0 + ext):** diffuse competition collapses retrieval (competition, not length — **§1.8→80**: pillar B ("not length") now **cross-provider to ~1M** — DeepSeek **v4-pro** neutral holds 1.00@94k like Haiku, only a mild length effect @758k (`results-length-extension.md`); the *diffuse-collapse* cross-structure/provider generality is still NOT established, clause (b) open). KV-cache positional cost gradient, tool change worst (§1.1→80, §3.3, 7× cache root). Cross-family (DeepSeek) diffuse-collapse inconclusive — structure×needle-similarity entanglement; DeepSeek adds **abstention** as a 3rd failure mode, now seen under length-stress too (§3.8, caveated). See `experiments/phase-1.0/results.md`, `results-exercise-B.md`, `results-cross-family.md`, `results-length-extension.md`.
- **Current phase:** **Phase 2.0 Thread B** (Module 3, Reasoning patterns) — **closing (retro + `/learn` + three-reviewer pass + a confound-removing re-test all done; pending user merge).** Built a position-forming harness (`src/stance/reasoning/`: sampler → 4 arms [baseline/react/plan_execute/reflection] → fuzzy 5-criterion Sonnet judge → runner + slotted-distractor multiplier; TDD, `make check` green, 229 tests). **`reasoning-pattern` → candidate 63 (§1.9)**: *no universal best reasoning pattern; effectiveness = bias × epistemic-structure.* **The critical-boundary review did its job twice, then cross-provider firmed it** — the three-reviewer pass caught both headline mechanisms as artifacts; a confound-removing re-test (anonymized presentation defeats triage; neutral critique; prose-EVIDENCE_USE) discriminated (**reflection = systematic caution EARNED**; **plan-execute "commit-to-few" REFUTED** as teaser-triage → plan ≈ react; **retrieve>stuff-under-dilution survives but generic**; **§1.1 null**); then **Kimi K3 (2.8T flagship) replicated both mechanisms** → single-provider discharged. Trajectory **40→62→50→58→63** (prereg → over-claim → pass → re-test → cross-provider). Path to *active*: a **human-validated grader** + a **2nd debate per regime**. New process lessons **§0.25** (distractors must be on-axis + non-decisive-by-flaw, else triaged → no competition) and the **soft-defer-that's-also-a-position lapses** finding (→ curriculum-coverage audit + a back-fill phase 1.2 planned).
...
- **Experiment substrate: DeepSeek-primary, Claude-anchor** (adopted 2026-06-05, Phase 1.0 ext). Measurement experiments default to **DeepSeek v4-flash** as the primary model (**v4-pro** for capability-sensitive cells); **Claude** (Haiku/Sonnet/Opus, via `--provider anthropic`) is used only for (a) **spot-anchors** — a few cells per experiment to confirm the mechanism transfers and calibrate magnitude (§0.8) — and (b) experiments whose **object of study is a Claude-only feature** (prompt-caching semantics / Exercise B, structured outputs, compaction, `count_tokens`). DeepSeek runs go through the **Anthropic-compatible endpoint** (`api.deepseek.com/anthropic`) to reuse the harness, counting tokens on Anthropic-Haiku for a consistent x-axis; **verify the declared-tools path with a smoke test when first building real agents (Module 2)** — fall back to DeepSeek's native OpenAI-format client if the shim leaks tool-call markup (lessons §0.17). **Re-validate the measurement on DeepSeek per new experiment type** (scorer phrasings, response/usage shapes — §0.16/§0.17). Synthesis states the substrate explicitly: positions claim *cross-provider* mechanisms, DeepSeek-measured + Claude-anchored. *Trigger: without this, a years-horizon, curiosity-driven curriculum is unaffordable on Haiku ($1/$5 per MTok) and the evidence base is single-provider (Anthropic selection bias); with this, broad exploration is ~7–18× cheaper (v4-flash $0.14/$0.28) AND positions are cross-provider by construction.* (Distinct from the **subagent** model-tiering in § Git Lifecycle, which is about Claude Code reviewer/task/mechanical subagents, billed via Claude Code — not the `.env` experiment budget.)

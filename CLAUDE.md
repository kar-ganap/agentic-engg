...
- **Current phase:** **Phase 1.1** (Module 2, Tools) — **in progress.** Harness built (TDD, `make check` green, ~194 tests; domain/tools/loop/scorer/5 task tiers). **#4 resolved → demoted: #4(i) 60→25, #4 ⊆ §1.8** (`experiments/phase-1.1/results-4v2.md`): five self-generated-interference designs — four high-discriminability held (0 mis-binds: exact-key binding to 953k / role-binding / recency N=40 / two-phase rolebind N=16 buried 37k), the fifth (low-disc cue) collapsed as the §1.8 lure-capture **identically active vs passive** → self-generation is a structural accessibility advantage (≈ tautology), not a novel immunity. **#6 resolved → 45→62** (`results-6.md`): *remove the format choice, fix inline-detailed* — none of v4-flash/v4-pro/Sonnet spontaneously exploits an agent-set return-format choice (3 models, 2 families, ~7× span); handle-block is pure overhead; concise-pruned fails. **#4(ii) parked.** **#3 resolved → 70→78** (`results-3.md`): predict→verify closed — a carry-vs-swap tool-set break-even **EXISTS**, KV-cache-governed (predicted N\* ≈ 5,111 tool-tokens, verified 6,461, ~26%); **qualifies §1.1's cache leg** (don't-mutate is cheaper only while the tool universe is small); existence+cost-model cross-provider, *location* TTL/provider-dependent (Claude anchor deferred). **All three sub-experiments (#4/#6/#3) done → Phase 1.1 close next: retro → `/learn` → three-reviewer pass** (critical boundary: position commitments). Uncommitted: full #4-v2/#6/#3 arc (5 task tiers, 7 runners, 3 results files, synthesis/plan/spend updates).

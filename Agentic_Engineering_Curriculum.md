...
- **Evaluation-driven tool development:** generate realistic eval tasks, measure tool-use success, let an agent help optimize tool descriptions. In Anthropic's *Writing Effective Tools for AI Agents*, a tool-testing agent that repeatedly used flawed tools then rewrote their descriptions produced "a 40% decrease in task completion time for future agents using improved descriptions" <!-- [VERIFIED-FALSE 2026-06-09 — this quoted "40%" statistic is NOT in the cited blog (read firsthand: https://www.anthropic.com/engineering/writing-tools-for-agents); the blog quantifies nothing here, only describing the agent-rewrites-tools process. Treat as fabricated/mis-attributed. The blog's ACTUAL claim is qualitative (see next clause). §0.15/§0.16. Kept in place to preserve the error's provenance.] -->; the same post notes "Claude Sonnet 3.5 achieved state-of-the-art performance on the SWE-bench Verified evaluation after we made precise refinements to tool descriptions" <!-- [VERIFIED-PRESENT 2026-06-09 — this SWE-bench sentence IS in the blog, but it is QUALITATIVE: "...dramatically reducing error rates and improving task completion" — no number attached.] -->.

...
- **#4(i) — mechanism (rot exists; competition degrades retrieval in self-generated trajectories): prior 60.** Grounded in §1.8 + §0.8 (mechanism transfers), tempered because cross-substrate generalization is *exactly* what bit us in 1.0-ext (§0.18; clause-b open).
  ...
  - **POSTERIOR 60 → 25 (2026-06-14, #4-v2; `experiments/phase-1.1/results-4v2.md`).** Retraction met across **four high-disc designs** (correct-use flat, 0 mis-binds — binding/role-binding/recency/rolebind). A fifth **low-disc** design *did* collapse, but as the **§1.8 identification failure, identically active vs passive** → **#4 ⊆ §1.8**, not a separate effect. The 25 covers "rot can occur agentically" (= §1.8 applying). Self-generation is neither cause nor cure; the apparent immunity is a structural near-tautology (fetching ≠ wall-retrieval).
...
- **#6 — remove-the-format-choice: prior 45** (novel, zero prior evidence, genuine equipoise; contribution-worthy *because* uncertain).
  ...
  - **POSTERIOR 45 → 62 (2026-06-14; `experiments/phase-1.1/results-6.md`).** **Remove the choice; fix inline-detailed.** Load-bearing finding (behavioral, confound-free): **none of v4-flash, v4-pro, Sonnet exploits the choice** — all default to `detailed` everywhere, even on returns where `concise` is free (3 models, 2 families, ~7× capability span) → C is unexploited overhead. The token ordering *flips* across models → token DV confounded (behavioral signal does the work). A (inline-detailed) is cheapest + 100% success; D (handle-block) is pure overhead; B (concise, handle pruned) → 0%. Token ranking is *supporting only* (call-count confound). Caveat: success didn't discriminate (easy task); the choice may be *usable* under an economy-pressure prompt (untested) — the one revival path.

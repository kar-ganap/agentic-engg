...
**Confidence:** 58 (candidate). Legs: no-universal-winner **65** · reflection-systematic-caution **60** · retrieve-beats-stuff-under-dilution **55** · bias×structure **52** · regime-taxonomy **40**. Trajectory **40 (prereg) → 62 (first sweep, over-claimed) → 50 (three-reviewer pass) → 58 (confound-removed re-test)**. Capped by: single-provider (DeepSeek arms + Sonnet judge), n=5 (9/180 bad rows, all retrieve arms), 1–2 debates per mechanism, an unvalidated fuzzy grader.
...
**Status:** candidate. Registered 2026-06-29 (prereg 40); over-claimed to 62 on the first sweep; corrected to 50 by the three-reviewer pass (mechanism legs = artifacts); re-earned to 58 by the confound-removed re-test (reflection earned, plan commit-to-few refuted), 2026-07-26. Cross-provider replication + a second debate per regime are the open extensions.
---

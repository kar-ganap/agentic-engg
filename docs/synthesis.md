...
**Confidence:** 80 (raised 75→80, 2026-06-05, on own-substrate experimental confirmation of the *cache-economics* half — Phase 1.0 Exercise B; trimmed from a proposed 82 in the three-reviewer pass because only one of two legs is empirically discharged). **Sub-confidence split:** cache-economics leg ~90 (measured: tools are the cache root; mutating them is the worst anti-pattern, 7× a stable prefix); model-coherence leg ~65 (literature-only, Manus — untested here; the coherence experiment is a Phase 1.1 follow-on). The position's retraction criterion requires *both* legs to be null, so the untested leg caps the overall number.
...
smoke), empirically **6,461** (~26%; residual = measured swap floor +22% over the nominal target).
...
**Note — candidate *architectural* mechanism for position bias (Chowdhury, *Lost in the Middle at Birth: An Exact Theory of Transformer Position Bias*, arXiv:2603.10123v1, Meta, Mar 2026; primary source read 2026-06-05):** the paper derives that the **U-shaped positional bias is present at *initialization*** (untrained Qwen2/GPT-2 show it at step 0) and is an *inherent **geometric** property of the causal decoder with **residual connections*** — explicitly **NOT** a softmax artifact or RoPE distance-decay (it is *identical with or without RoPE*). Mechanism: causal masking → a logarithmic gradient-**influence** divergence at the start (*Primacy Tail*); residual connections → an isolated O(1) anchor at the final token (*Recency Delta*); between them a *factorial dead zone* O(1/(H−1)!) in depth H (the middle). **Standard training does NOT overcome it** — it persists as an architectural baseline (the paper doesn't claim it's insurmountable, only that overcoming it needs targeted intervention). **Hold loosely for §1.3, do not promote:** (i) it measures *influence/sensitivity*, not *accuracy* — distinct from both §1.3's active-influence and from retrievability; (ii) reconciles with Chroma's *no-NIAH-position-effect* not via "training mitigates the valley" but via learned **content-detectors** overriding the persistent architectural valley for easy retrieval, so the observed *accuracy* shape stays task-dependent; (iii) untestable on our Anthropic-API behavioral setup (needs open-weights interpretability — same regime as the §1.3-v2 open problem). The Primacy-Tail-vs-Recency-Delta *asymmetry* (diffuse log tail vs sharp O(1) anchor) loosely resonates with §1.3's "J, not symmetric U" — but it's the sensitivity quantity, so corroboration is weak. **Net:** a primary-source reason the position-*accuracy* shape is regime-specific; does **not** license a universal accuracy-U; **no confidence change** (wrong quantity + untestable here). **Correction (2026-06-05, three-reviewer pass):** an earlier version of this note — built from a fast-model PDF summary — wrongly attributed the bias to softmax+RoPE and said "training mitigates"; reading the source reversed *both* (geometric causal+residual; training does NOT overcome). Second instance of secondhand-summary error → verify the primary source (lessons §0.13). Reference class to round out (not yet a literature review): attention-sinks / StreamingLLM (Xiao et al. 2023), the near-twin arXiv:2502.01951, "Lost in the Middle: Emergent from IR Demands" (arXiv:2510.10276) — competing mechanistic accounts exist.
...
- **Experiment (agentic self-generated extension — Phase 1.1 #4-v2, 2026-06-14, `experiments/phase-1.1/results-4v2.md`):** the diffuse collapse is **provenance-blind**. Across four *high-discriminability* agentic designs (self-generated needles — exact-key/recency/role-binding/two-phase-rolebind) retrieval held (0 mis-binds); a *low-discriminability* cue induced the §1.8 **lure-capture identically whether the values were self-generated or passively dumped** (same scenario/seed, both arms). So §1.8's identification collapse appears in **self-generated agentic trajectories**, not just passive prose, and self-generation neither triggers nor prevents it (**#4 ⊆ §1.8**; the apparent "agentic immunity" is a structural near-tautology — fetching ≠ wall-retrieval). Magnitude uncommittable (the gradeable/luring sweet-spot, §0.18); recorded as a cross-setting scope note, **no confidence change**.
...
- **Up 5–10 (partially banked 2026-06-04):** replication on our own agentic substrate (§3.6) showing S/N predicts the knee better than length — *realised for one structure*; this is the 70→78 move. The remaining headroom to ~82/85 needs a second structure / cross-family.
- **Further up (toward 82/85):** a *clean* cross-structure + cross-family replication — needs a structure-invariant needle first (the 2026-06-05 attempt was confounded/entangled; see the generality caveat). Not a single run away.
...
Resolves the **cache-economics** sub-claim of §1.1 (→ §1.1 75→80). A *sharper measurement*, not new knowledge — the positional gradient **confirms documented prefix-cache mechanics** on the Anthropic API; it is **pedagogical / own-substrate, not a novel contribution** (correctly absent from `contribution-candidates.md`). **Prior art (three-reviewer pass, 2026-06-05; RadixAttention/vLLM cited from established knowledge, KVFlow read firsthand):** the "longest-common-prefix match, change cascades downward" design is **RadixAttention/SGLang** (radix-tree prefix reuse, arXiv:2312.07104) and **vLLM Automatic Prefix Caching** (block-hash prefix reuse, on PagedAttention, arXiv:2309.06180); Anthropic's prompt caching is a hosted instance. Agentic prefix caching is now an active area (e.g. **KVFlow**, arXiv:2507.07400 — *eviction/scheduling* for multi-agent workflows, which *assumes* prefix reuse of agents' fixed prompts rather than establishing the mechanism). Measurement caveat: the shared 5-min server cache bleeds across conditions (the `tools` root especially) → per-condition content isolation is needed (lessons §0.14).
...
- *Competition (relatedness distribution — the key axis, added 2026-06-03):* `neutral` (topically-unrelated filler — Chroma's bulk) · `localized(n)` (a few discrete similar-but-wrong distractors in neutral bulk — Chroma's distractor design) · `diffuse(density)` (the *whole* haystack task-related, so competition is pervasive — the **real agentic regime**). **Prior-art correction (three-reviewer pass, 2026-06-05):** the bare fixed↔length-scaling distractor *count* axis is **NOT novel** — RULER (Hsieh et al., arXiv:2404.06654) already spans it (Multi-key NIAH: subtask-1 = 4 fixed distractors ≈ localized; subtask-2/3 = `num_keys = full haystack` ≈ diffuse), and its headline ("perfect vanilla NIAH, large drops with length; failure to ignore distractors") is ours. Cite RULER; do not claim localized↔diffuse as the contribution. *What RULER does NOT do (our verified delta, read firsthand):* (i) **distractor relatedness/potency as a swept knob** — RULER uses same-format random distractors, "does not vary distractor similarity"; (ii) an **agentic-structure substrate** (tool-call streams) vs RULER's noise/essays; (iii) a **committed (confident-usable) vs mentioned** collapse (discriminability before burial). Those three are the contribution.
...
- **Validation spot-checks:** *Sonnet* — **DONE 2026-06-04**; replicates the collapse (zero by ~5k, earlier/harder than Haiku) → not a Haiku artifact; discharges the within-family leg of §1.8's retraction clause (b). *Realism (LLM-generated competitors)* — **DONE 2026-06-05**: natural-phrasing potent competitors reproduce the collapse (knee ~50k, still falling to 0.20 @100k) → **not a templating artifact**; the knee is *potency/composition-dependent* (templated-saturated ~10k < natural-mixed ~50k), so the specific small budget is the saturated case, not universal (§5.2 qualified). Monotone potency ordering = bonus §1.8 evidence. v1 of this check was a realism×relatedness confound (lesson logged). *Cross-provider (DeepSeek) + cross-structure (clean_essay)* — **ATTEMPTED 2026-06-05, INCONCLUSIVE** (`results-cross-family.md`): DeepSeek on `tool_call_stream` is confounded (it emits tool-call markup instead of answering — the history primes tool-call continuation; its diffuse degradation shows as *abstention*, not the discriminability collapse); and the provider-neutral `clean_essay` substrate can't isolate the effect (low-sim wrecks the control via folio-wrinkle, high-sim too easy). Net: the effect is **entangled with structure × needle-similarity** → cross-structure/provider generality **not established**; clause (b) open. A structure-invariant needle is the prerequisite for a clean test. **P3 (relatedness sweep) partly previewed** by the realism dose-response; the controlled sweep remains for §5.
...
**Stance (provisional, candidate hypothesis):** As model capability increases, the failure mode under diffuse competition shifts from **confident confabulation** (emit a wrong / multiple catalog number) toward **honest refusal** (decline / flag the records as conflicting), and the diffuse knee moves **earlier**, not later. *One reading:* a stronger model is a better *conflict-detector* (refuses under detected ambiguity) rather than a more robust *retriever* — so on a committed-accuracy metric it looks *worse* while behaving more appropriately. **An equally live reading is that the explicit `UNKNOWN` affordance, not capability, drives Sonnet's refusal (see literature note) — undecided pending the affordance-control.** Hold the framing to "consistent with conflict-detection *or* refusal-affordance-following," not a settled mechanism.
...
- **Cross-family data point (DeepSeek-v4-flash, 2026-06-05, heavily caveated — `results-cross-family.md`):** under diffuse, DeepSeek's failure mode is **abstention** (empty / leaked tool-call attempt; empty-rate 24% diffuse vs 2% neutral) — a *third* mode distinct from Haiku-confabulate and Sonnet-refuse-with-text. **But** cross-family confounds capability with training (precondition), and the measurement is confounded by DeepSeek's tool-call continuation — so this does **not** advance §3.8; it's a suggestive third point, not evidence for the capability ladder.
...
**Relation to literature (three-reviewer pass, 2026-06-05 — sources read firsthand):** this **contradicts**, rather than re-derives, the parametric-uncertainty literature. *From Loops to Oops* (arXiv:2407.06071) and OpenAI's *Why Language Models Hallucinate* both find the **opposite** scaling — larger / instruction-tuned models hallucinate **more** and abstain **less** (and abstention-instructions usually *fail*). But that work studies *parametric* knowledge gaps (the fact is absent); our regime is *in-context diffuse competition* (the answer is present but non-discriminable). So §3.8 is a candidate **regime-dependent reversal** of the established direction — higher-value if real, but it now carries a **higher burden of proof**, and the affordance-control is **mandatory, not optional** (the literature makes the `UNKNOWN` affordance the prime suspect for Sonnet's refusal). The fallback taxonomy there is repetition→degenerate→hallucination (abstention is *not* a rung) — so our "ladder" framing should cite it as *contrasting*, not as the source. Cite as contradicting evidence; engage the abstention/selective-prediction class (Know-Your-Limits survey, TACL 2024) for the committed-penalizes-refusal point.
...
### arXiv:2603.10123 — Chowdhury, *Lost in the Middle at Birth: An Exact Theory of Transformer Position Bias* (Meta, Mar 2026) — read (primary source) 2026-06-04; **re-read + corrected 2026-06-05**

**Headline (corrected against the actual PDF):** the **U-shaped positional bias is present at *initialization*** (untrained Qwen2/GPT-2 at step 0) and is an *inherent **geometric** property of the causal decoder with **residual connections*** — **explicitly not** softmax artifacts or RoPE distance-decay (*identical with or without RoPE*). Mechanism via causal attention as iterated Cesàro-matrix powers: causal masking → logarithmic gradient-influence divergence at the start (**Primacy Tail**); residual → an isolated O(1) anchor at the last token (**Recency Delta**); a **factorial dead zone** O(1/(H−1)!) in depth H between them. **Standard training does NOT overcome it** (persists as an architectural baseline; not claimed insurmountable). It measures *influence/sensitivity*, not task accuracy; autoregressive-decoder scope.

**Why read it / the lesson:** raised via a Gemini summary that overstated it; a first WebFetch (fast-model PDF summary) then **mis-attributed** the mechanism to softmax+RoPE and said "training mitigates." The three-reviewer pass flagged the title; reading the actual PDF reversed **both** attribution claims (geometric causal+residual; training does *not* overcome). **Two-stage secondhand error** (Gemini, then a summarizer) → strong vindication of verify-the-primary-source (lessons §0.13).

**What it moves:** a candidate *architectural* mechanism for §1.3 (see §1.3 note). Held **loosely** — sensitivity≠accuracy; reconciles with Chroma's no-NIAH-position-effect via learned *content-detectors* overriding the persistent valley (not via training mitigating it); untestable on our API-behavioral setup. Strengthens §1.3's "regime-specific shape" precondition; no confidence change. Reference class to round out: attention-sinks/StreamingLLM (Xiao et al. 2023), arXiv:2502.01951, arXiv:2510.10276 — competing accounts exist.
...
> **Confirmation (Phase 1.0 Exercise A, `experiments/phase-1.0/results.md`):** modelling the *diffuse* regime (whole haystack task-related — the agentic case) on `tool_call_stream`, the **free budget collapses** — confident retrieval → 0 — while the *neutral* control holds a passband flat to 100k. Depth-decomposed: the collapse is uniform across needle position (no position effect → competition-, not position-driven). So the agentic "knee→0" prediction holds on our own substrate. Re-promoted from "Chroma-grounded prediction" to "own-substrate-confirmed."
>
> **Qualification (realism check, 2026-06-05) — the *knee* is potency-dependent, not a universal number.** The knee depends on competitor relatedness/composition: ≈10–20k (Haiku) / ≈5k (Sonnet) for the **templated/saturated** case (uniform, ~75% rival *same-attribute* values), but ≈50k for **natural-phrasing mixed-potency** competitors (LLM-generated pool) — which still collapses (0.20 @100k), just later. So the *phenomenon* (diffuse competition collapses the budget) is robust to phrasing and composition; the **specific small budget (~10–20k) is the saturated case, not universal.** Monotone in potency (low-relatedness pool: no collapse < mixed natural: knee ~50k < saturated templated: knee ~10k) — clean §1.8 evidence. Slope (templated): −0.57/decade diffuse vs −0.015 neutral (Haiku), ~38× steeper. Caveat: a unique-string needle is a *retrievability ceiling* (§1.3) — active-influence budget is plausibly ≤ this; and whether *real* agentic contexts run saturated or mixed relatedness is open (→ `research_doc_stream`, §5 relatedness sweep).

...
> **⚠️ INTERIM (three-reviewer pass, 2026-07-25) — downgraded to hypothesis, conf 62→50.** The pass found the two *mechanism* legs are **artifacts**: plan-execute's §1.8 "commit-to-few" win is **teaser-triage** (it reads exactly the teaser-identifiable targets — §0.25 stopped the *stuff* arms triaging, never the *retrieve* arms), and reflection's §3.8 win is **confounded by the critique prompt naming "overclaim."** EVIDENCE_USE is invalid between arms (scores citation-purity, an access-mode artifact). §1.1 is a **null**, not a regime → "2 regimes + 1 null." Arms are **ReAct / ReWOO-Plan-and-Solve / Self-Refine** (the "reflection" arm is Self-Refine, Madaan 2023 — not Reflexion). Only **no-universal-winner (via the failure regimes)** survives robustly (~60). A **teaser-matched + neutral-critique + prose-EVIDENCE_USE re-test** is underway to re-earn or drop the mechanism legs; the prior-art paragraph + full rewrite land with those results. Interim legs: sig-density-dilution 40 / bias×structure 40 / reflection-caution 40 / regime-taxonomy 35. See `docs/phases/phase-2.0-retro.md` § three-reviewer pass. *The stance below is the pre-review draft, retained for the trajectory.*
